...
# MAGIC %md
# MAGIC ## Stats on broad diags (most prevalent ones) - on flagged cohorts
...
FEATURE_TYPE = BROAD_FEATURE_TYPE # must be one of SPECIFIC_FEATURE_TYPE or BROAD_FEATURE_TYPE
DB_CONFIG = DIAGNOSES
FLAGGED = True

# COMMAND ----------

MAIN_COHORTS = ["cohort", "all_cohorts"]
OTHER_COHORTS_LIST = [["like_cohort", "control_cohort"], []]
LIST_OF_COLS_TO_GROUP_ON = [[COL_ICD_MEDICAL_GROUP], [COL_ICD_MEDICAL_TERM, DB_CONFIG["COL_INFO"], COL_ICD_MEDICAL_GROUP]]

## LIST_OF_COLS_TO_GROUP_ON must be a list of columns to group on. COLS_TO_GROUP_ON must be a list of elements among: COL_ICD_MEDICAL_TERM, COL_ALGO_MEDICAL_TERM, COL_ALGO_MEDICAL_GROUP, COL_ALGO_MEDICAL_BLOCK (by descending precision). The first element of the list being the highest level of precision

## Reminder : COL_ICD_MEDICAL_GROUP is a grouping based on the truncation to 3 digits of the ICD codes. The truncation (and grouping) can be made to 4 or even 5 digits. It can be done by modifying the paramater GENERIC_N_DIGITS in the function 'get_processed_features' in the file main/2_feature_creation/2_1_process_broad_features
...
for algo_name in ALGO_OF_INTEREST:
    ...
    print("--------------------------")
    print("")
    for MAIN_COHORT, OTHER_COHORTS in zip(MAIN_COHORTS, OTHER_COHORTS_LIST):
            ...
            if FLAGGED:
                if MAIN_COHORT=="all_cohorts":
                    write_spark_file(
                        ...
                        f"{algo_name}_stats_{FEATURE_TYPE}_{COL_STATS_LEVEL}_diags_results_{TIMERANGE}_all_flagged",
                        file_type="csv",
                    )
                else:
                    write_spark_file(
...
COL_STATS_LEVEL = COL_ICD_MEDICAL_GROUP # For specific diags, must be an element among: COL_ICD_MEDICAL_TERM, COL_ICD_MEDICAL_GROUP, COL_ALGO_MEDICAL_TERM, COL_ALGO_MEDICAL_GROUP, COL_ALGO_MEDICAL_BLOCK (by descending precision)
